...
Для развертывания необходимо с помощью angular cli запустить проект (ng serve).  
Так же необходимо запустить json server (json-server --watch src/database/documents.json).
